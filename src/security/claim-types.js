'use strict'

/**
 * Default claim types for users that could be injected in token or stored to aux client application.
 */
class ClaimTypes {
  static get Actor() {
    return 'http://schemas.xmlsoap.org/ws/2009/09/identity/claims/actor'
  }
  static get Anonymous() {
    return 'http://schemas.xmlsoap.org/ws/2005/05/identity/claims/anonymous'
  }
  static get Authentication() {
    return 'http://schemas.xmlsoap.org/ws/2005/05/identity/claims/authenticated'
  }
  static get AuthenticationInstant() {
    return 'http://schemas.microsoft.com/ws/2008/06/identity/claims/authenticationinstant'
  }
  static get AuthenticationMethod() {
    return 'http://schemas.microsoft.com/ws/2008/06/identity/claims/authenticationmethod'
  }
  static get AuthorizationDecision() {
    return 'http://schemas.xmlsoap.org/ws/2005/05/identity/claims/authorizationdecision'
  }
  static get CookiePath() {
    return 'http://schemas.microsoft.com/ws/2008/06/identity/claims/cookiepath'
  }
  static get Country() {
    return 'http://schemas.xmlsoap.org/ws/2005/05/identity/claims/country'
  }
  static get DateOfBirth() {
    return 'http://schemas.xmlsoap.org/ws/2005/05/identity/claims/dateofbirth'
  }
  static get DenyOnlyPrimaryGroupSid() {
    return 'http://schemas.microsoft.com/ws/2008/06/identity/claims/denyonlyprimarygroupsid'
  }
  static get DenyOnlyPrimarySid() {
    return 'http://schemas.microsoft.com/ws/2008/06/identity/claims/denyonlyprimarysid'
  }
  static get DenyOnlySid() {
    return 'http://schemas.xmlsoap.org/ws/2005/05/identity/claims/denyonlysid'
  }
  static get DenyOnlyWindowsDeviceGroup() {
    return 'http://schemas.microsoft.com/ws/2008/06/identity/claims/denyonlywindowsdevicegroup'
  }
  static get Dns() {
    return 'http://schemas.xmlsoap.org/ws/2005/05/identity/claims/dns'
  }
  static get Dsa() {
    return 'http://schemas.microsoft.com/ws/2008/06/identity/claims/dsa'
  }
  static get Email() {
    return 'http://schemas.xmlsoap.org/ws/2005/05/identity/claims/emailaddress'
  }
  static get Expiration() {
    return 'http://schemas.microsoft.com/ws/2008/06/identity/claims/expiration'
  }
  static get Expired() {
    return 'http://schemas.microsoft.com/ws/2008/06/identity/claims/expired'
  }
  static get Gender() {
    return 'http://schemas.xmlsoap.org/ws/2005/05/identity/claims/gender'
  }
  static get GivenName() {
    return 'http://schemas.xmlsoap.org/ws/2005/05/identity/claims/givenname'
  }
  static get GroupSid() {
    return 'http://schemas.microsoft.com/ws/2008/06/identity/claims/groupsid'
  }
  static get Hash() {
    return 'http://schemas.xmlsoap.org/ws/2005/05/identity/claims/hash'
  }
  static get HomePhone() {
    return 'http://schemas.xmlsoap.org/ws/2005/05/identity/claims/homephone'
  }
  static get IsPersistent() {
    return 'http://schemas.microsoft.com/ws/2008/06/identity/claims/ispersistent'
  }
  static get Locality() {
    return 'http://schemas.xmlsoap.org/ws/2005/05/identity/claims/locality'
  }
  static get MobilePhone() {
    return 'http://schemas.xmlsoap.org/ws/2005/05/identity/claims/mobilephone'
  }
  static get Name() {
    return 'http://schemas.xmlsoap.org/ws/2005/05/identity/claims/name'
  }
  static get NameIdentifier() {
    return 'http://schemas.xmlsoap.org/ws/2005/05/identity/claims/nameidentifier'
  }
  static get OtherPhone() {
    return 'http://schemas.xmlsoap.org/ws/2005/05/identity/claims/otherphone'
  }
  static get PostalCode() {
    return 'http://schemas.xmlsoap.org/ws/2005/05/identity/claims/postalcode'
  }
  static get PrimaryGroupSid() {
    return 'http://schemas.microsoft.com/ws/2008/06/identity/claims/primarygroupsid'
  }
  static get PrimarySid() {
    return 'http://schemas.microsoft.com/ws/2008/06/identity/claims/primarysid'
  }
  static get Role() {
    return 'http://schemas.microsoft.com/ws/2008/06/identity/claims/role'
  }
  static get Rsa() {
    return 'http://schemas.xmlsoap.org/ws/2005/05/identity/claims/rsa'
  }
  static get SerialNumber() {
    return 'http://schemas.microsoft.com/ws/2008/06/identity/claims/serialnumber'
  }
  static get Sid() {
    return 'http://schemas.xmlsoap.org/ws/2005/05/identity/claims/sid'
  }
  static get Spn() {
    return 'http://schemas.xmlsoap.org/ws/2005/05/identity/claims/spn'
  }
  static get StateOrProvince() {
    return 'http://schemas.xmlsoap.org/ws/2005/05/identity/claims/stateorprovince'
  }
  static get StreetAddress() {
    return 'http://schemas.xmlsoap.org/ws/2005/05/identity/claims/streetaddress'
  }
  static get Surname() {
    return 'http://schemas.xmlsoap.org/ws/2005/05/identity/claims/surname'
  }
  static get System() {
    return 'http://schemas.xmlsoap.org/ws/2005/05/identity/claims/system'
  }
  static get Thumbprint() {
    return 'http://schemas.xmlsoap.org/ws/2005/05/identity/claims/thumbprint'
  }
  static get Upn() {
    return 'http://schemas.xmlsoap.org/ws/2005/05/identity/claims/upn'
  }
  static get Uri() {
    return 'http://schemas.xmlsoap.org/ws/2005/05/identity/claims/uri'
  }
  static get UserData() {
    return 'http://schemas.microsoft.com/ws/2008/06/identity/claims/userdata'
  }
  static get Version() {
    return 'http://schemas.microsoft.com/ws/2008/06/identity/claims/version'
  }
  static get Webpage() {
    return 'http://schemas.xmlsoap.org/ws/2005/05/identity/claims/webpage'
  }
  static get WindowsAccountName() {
    return 'http://schemas.microsoft.com/ws/2008/06/identity/claims/windowsaccountname'
  }
  static get WindowsDeviceClaim() {
    return 'http://schemas.microsoft.com/ws/2008/06/identity/claims/windowsdeviceclaim'
  }
  static get WindowsDeviceGroup() {
    return 'http://schemas.microsoft.com/ws/2008/06/identity/claims/windowsdevicegroup'
  }
  static get WindowsFqbnVersion() {
    return 'http://schemas.microsoft.com/ws/2008/06/identity/claims/windowsfqbnversion'
  }
  static get WindowsSubAuthority() {
    return 'http://schemas.microsoft.com/ws/2008/06/identity/claims/windowssubauthority'
  }
  static get WindowsUserClaim() {
    return 'http://schemas.microsoft.com/ws/2008/06/identity/claims/windowsuserclaim'
  }
  static get X500DistinguishedName() {
    return 'http://schemas.xmlsoap.org/ws/2005/05/identity/claims/x500distinguishedname'
  }
}

module.exports = ClaimTypes
